...
def parse_args():
    ...
    if os.path.isdir(call_args['inputs']):
        image_path = call_args['inputs'].split('image')[1].strip('/')
        call_args['out_dir'] = os.path.join(call_args['out_dir'], image_path)
    else:
        image_path = call_args['inputs'].split('image')[1].strip('/')
        call_args['out_dir'] = os.path.join(call_args['out_dir'], os.path.dirname(image_path))
